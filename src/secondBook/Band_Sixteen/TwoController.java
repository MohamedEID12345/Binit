...
firstBook.Band_One.OneController one=new firstBook.Band_One.OneController();
    ...
    private void True1(MouseEvent event) {
        draggablemaker.trueAction2("/secondBook/Band_Sixteen/Three.fxml", event);
    }

    @FXML
    private void False1(MouseEvent event) {
        one.paneContentMessage2(ones);
        //open new scane
    Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(3), ev -> {
     draggablemaker.falseAction2("/secondBook/Band_Sixteen/Three.fxml", event);
    }));
    timeline.play();
    //end method
    }
